...
async def create_room(guild: discord.Guild, member: discord.Member):
    ...
    if not category or not isinstance(category, discord.CategoryChannel):
        log.error(f"Không tìm thấy danh mục ID {DEFAULT_CATEGORY_ID}")
        return None
...
async def on_voice_state_update(member: discord.Member, before: discord.VoiceState, after: discord.VoiceState):
    if after.channel and isinstance(after.channel, discord.VoiceChannel):
        ...
        if row and after.channel.id == row["generator_id"]:
            if after.channel.category_id == DEFAULT_CATEGORY_ID:
...
# Slash Commands
# -----------------------------
@bot.tree.command(name="setup", description="Khởi tạo kênh Tạo Phòng trong danh mục quy định")
