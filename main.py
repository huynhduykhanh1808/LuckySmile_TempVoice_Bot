...
# Slash Commands
# -----------------------------
@bot.tree.command(name="setup", description="Khởi tạo hoặc tạo lại danh mục và kênh Tạo Phòng cho Bot")
...
async def setup(interaction: discord.Interaction):
    ...
    await interaction.followup.send(
        f"✅ **Đã hoàn tất cài đặt thành công!**\n"
        f"• **Danh mục:** {cat.name}\n"
        f"• **Kênh tạo phòng:** {gen.mention}\n"
        f"• **Kênh quản lý:** {ctrl.mention if ctrl else 'Không có'}",
        ephemeral=True
    )
...
async def on_voice_state_update(member: discord.Member, before: discord.VoiceState, after: discord.VoiceState):
    # 1. Người dùng tham gia kênh Tạo Phòng -> Tạo phòng thoại riêng
    ...
    # 2. Người dùng rời kênh thoại -> Xóa phòng nếu phòng đó trống
    if before.channel and isinstance(before.channel, discord.VoiceChannel):
        row = get_generator(member.guild.id)

        # KHÔNG BAO GIỜ XÓA KÊNH TẠO PHÒNG
        if row and before.channel.id == row["generator_id"]:
            return

        # Chỉ xóa phòng nằm trong CSDL khi không còn ai trong phòng
